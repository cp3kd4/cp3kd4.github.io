...
const generateItem = (_id, _text) => {
  ...
  inputTag.id = _id;
  inputTag.addEventListener('click', event => toggleTodo(event.currentTarget.id, event.currentTarget.checked))
...
const viewTodos = () => {
  ...
  else if(clearButton.parentNode === clearDiv) clearDiv.removeChild(clearButton);
  let filteredTodos;
  switch(state) {
    case ALL:
      filteredTodos = todos;
